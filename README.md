...
It is a desktop app on Windows and macOS, and — for the same artist away from the
workstation — an installable phone app that searches the same catalog offline and writes
byte-for-byte the same files. See [Mobile](#mobile-the-same-catalog-on-a-phone).
...
`dist-mobile/` is a plain static site: deployable anywhere, at any path, with no rebuild
between them — `base` is relative for exactly that reason. Nothing generated by
`mobile:data` is committed; it is all rebuildable from the catalog, and the index alone
changes with every harvest.

### Hosting

The site lives on **Cloudflare Pages** at `https://gis-browser.pages.dev`.

That is not a preference, it is a requirement of the Android wrapper below. A Trusted Web
Activity only drops the URL bar if Chrome finds `/.well-known/assetlinks.json` **at the
origin root**. A GitHub *project* page is served from `username.github.io/gis-browser/`,
whose origin root belongs to the user site rather than to this repo — so an app pointed at
a project page can never be verified, and shows a browser chrome bar forever.

```bash
npx wrangler login          # once, in your own browser
npm run mobile:deploy       # build, then push dist-mobile to Cloudflare Pages
```

**Deploys come from a developer machine, never from CI.** The site cannot be built in CI at
all: the search index is generated from the 2 GB harvested catalog, which exists only where
a harvest has been run. That is why `build.yml` does not deploy, and why the generated data
is gitignored rather than committed to make CI able to.

### Android: the TWA wrapper

[Bubblewrap](https://github.com/GoogleChromeLabs/bubblewrap) wraps the deployed PWA in a
Trusted Web Activity — a real Android package, installable and Play-publishable, that runs
the same site full screen with no browser UI.

`android/twa-manifest.json` is the **only committed source**. The entire Gradle project is
regenerated from it, so the generated tree is not committed:

```bash
npm run twa:update       # regenerate the Android project from twa-manifest.json
npm run twa:build        # build the signed APK and AAB
```

`twa:update` takes the version from `package.json`, so the Android `versionName` cannot
drift from the app it wraps, and lets bubblewrap increment `versionCode` — which is the
rule Play enforces between uploads. That increment is written back into
`android/twa-manifest.json`, which is exactly why that file is committed: the next release
has to know what the last one used.

Order matters here, and the reason is not obvious: `twa:update` **fetches the icons over
HTTP** from the URLs in the manifest, to rasterise them into the project. The site has to be
deployed before the app can be generated, not after.

Two steps are deliberately not automated, because both involve credentials:

- **`npx wrangler login`** authenticates against your own Cloudflare account, in your own
  browser.
- **The signing keystore** is yours to create, and its password must never be scripted:

  ```bash
  keytool -genkeypair -v -keystore android/android.keystore -alias upload \
          -keyalg RSA -keysize 2048 -validity 10000
  ```

  `android/android.keystore` and the `.keystore.json` bubblewrap writes beside it are
  gitignored, and must stay that way. **Lose that key and no existing install can ever be
  updated; publish it and anyone can ship an update to your users.**

Then the verification handshake, which is what makes it a TWA rather than a browser tab
with an icon:

```bash
npm run twa:assetlinks -- <SHA-256 fingerprint>   # writes .well-known/assetlinks.json
npm run mobile:deploy                             # redeploy, so the origin serves it
```

Chrome fetches `https://gis-browser.pages.dev/.well-known/assetlinks.json` on launch and
checks that it names both the installed package and the certificate it was **actually**
signed with. Every way that check can fail, fails identically and silently: the app opens,
the site works, and there is a browser chrome bar across the top forever. So `mobile:deploy`
warns when it is about to publish a build with no asset links — it only warns, because the
first deploy necessarily happens before any of this exists.

**If you publish through Google Play, the fingerprint here is Google's, not yours.** Play
App Signing re-signs your upload with a key Google holds, so the certificate on the
installed app is not the one in your keystore. Take the SHA-256 from Play Console → Setup →
App integrity. Using the upload key's fingerprint instead is the most common reason a TWA
that verified fine when sideloaded shows a URL bar once installed from Play — and since
both are real fingerprints of real keys, nothing anywhere tells you which one is wrong.

For a sideloaded build, it is the keystore's own:

```bash
keytool -list -v -keystore android/android.keystore -alias upload | grep SHA256
```
...
The Android app adds no source at all. Bubblewrap generates the entire Gradle project from
one committed JSON file, so there is no Java or Kotlin in this repository and nothing to
keep in sync — the app is the website, wrapped.
...
- **The site is not deployed, so the Android app is not verified.** The TWA toolchain is
  proven as far as it can be without an origin: bubblewrap generates the project from
  `android/twa-manifest.json` and Gradle builds a real 5.1 MB APK, with the right package
  id and the Trusted Web Activity launcher in its manifest. What has *not* happened is
  everything downstream of a live `https://gis-browser.pages.dev` — no deploy, no signing
  key, no `assetlinks.json`, and so no verified TWA and no install on a device. Until then
  the app would open the site with a URL bar across the top, which is a browser tab with an
  icon rather than an app.
